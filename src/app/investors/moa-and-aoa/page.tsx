// app/moa-and-aoa/page.tsx
'use client';

import React, { useState } from 'react';
import Navbar from '../../../components/layout/Navbar';
import './MOAandAOA.css';

const MOAandAOAPage = () => {
  // State for MOA and AOA accordion
  const [openDocuments, setOpenDocuments] = useState(true);

  return (
    <div className="moa-aoa-page">
      <Navbar />

      {/* Main Content Section with padding to account for navbar */}
      <div className="pt-[88px]">
        <div className="et_pb_section et_pb_section_1 et_section_regular">
          <div className="et_pb_row et_pb_row_0">
            <div className="et_pb_column et_pb_column_4_4 et_pb_column_0">
              
              <div className="et_pb_text et_pb_module et_pb_bg_layout_light et_pb_text_align_left et_pb_text_0">
                <div className="et_pb_text_inner">
                  <h1>MOA and AOA</h1>
                </div>
              </div>

              <div className="et_pb_module et_pb_accordion et_pb_accordion_0">
                
                {/* MOA and AOA Accordion */}
                <div className={`et_pb_module et_pb_toggle ${openDocuments ? 'et_pb_toggle_open' : 'et_pb_toggle_close'} et_pb_accordion_item_0`}>
                  <h5 className="et_pb_toggle_title" onClick={() => setOpenDocuments(!openDocuments)}>
                    Memorandum of Association (MOA) &amp; Articles of Association (AOA)
                  </h5>
                  <div className="et_pb_toggle_content clearfix">
                    <ul>
                      <li>
                        <a href="https://www.gourmetgateway.co.in/wp-content/uploads/2025/04/MOA.pdf" target="_blank" rel="noopener noreferrer">
                          Memorandum of Association (MOA)
                        </a>
                      </li>
                      <li>
                        <a href="https://www.gourmetgateway.co.in/wp-content/uploads/2025/04/AOA-ICVL.pdf" target="_blank" rel="noopener noreferrer">
                          Articles of Association (AOA) - ICVL
                        </a>
                      </li>
                    </ul>
                  </div>
                </div>

              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MOAandAOAPage;